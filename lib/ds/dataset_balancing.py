...
class BalancingDataset(Dataset):
    ...
    def __init__(self, base_set: Dataset, length_per_class=500):
        self._base_set = base_set
        self._length_per_class = length_per_class

        self._samples_by_class = {cls: [] for cls in range(0, 7)}
    ...
    def __len__(self):
        return self._length_per_class * 7

    def __getitem__(self, index) -> tuple[list[int], int]:
        if index >= len(self):
            raise IndexError
        cls = random.randrange(0, 7)
